...
//		
	...
	List<Integer> oddlist	=evenList.stream().map(i-> i*i).sorted().collect(Collectors.toList());
	
	System.out.println(oddlist);
//	oddlist.stream().map(i -> i)
	int sum=oddlist.stream().reduce(0, Integer::sum);
	...
	//oddlist.stream().map(i->i.sum(i, i+1)).forEach(t->System.out.println("sum of all elements of list"+t));
 	...
 	ls4.stream().map(i->i*2).collect(Collectors.toList()).forEach(System.out::println);;
//		System.out.p;rintln(ls5)
